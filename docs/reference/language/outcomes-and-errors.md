...
`throw` produces a language error; `check` can raise one when an upstream stage did not succeed. A refusal means a required authority, host guarantee, or supported route was unavailable. It does not mean a task ran and returned a failing status. An effectful ordinary source invocation is refused before adapter access; an accepted project task may reach a controlled host if its exact grants and enforcement allow it.
