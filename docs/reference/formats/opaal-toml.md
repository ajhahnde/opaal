...
`project.name` is an OPAAL identifier. `root_module` names the source file that may export tasks; `required_opaal` is a SemVer range checked against the running toolchain. `paths.root` and `paths.evidence` are project-contained paths. The example range admits compatible 1.x releases; a deployed project can choose a narrower requirement.
