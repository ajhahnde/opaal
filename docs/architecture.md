...
Loading is analysis, not execution. The module program contains the canonical graph, source files, parsed syntax, and deterministic local/export/import-name tables. Explicit `export { name }` lists can expose top-level lexical declarations and functions; `import { name } from '<path>'` resolves only names explicitly exported by the canonical target. Unknown, private, duplicate, and colliding names fail analysis with source-anchored diagnostics, and there is no wildcard import path.

Non-interactive `fsh <script>` execution injects the host filesystem adapter at this boundary, reports grouped excerpts for diagnostics that span multiple registered sources, and executes only the analyzed root syntax. Its load-only import declarations are skipped during that root execution, and imported module initialization remains unexecuted. Named-import analysis does not yet materialize runtime values, so named imports remain unavailable to script execution until a later M8.2 runtime boundary defines initialization and binding behavior. Other frontends and full reference resolution remain later layers over the shared program.
