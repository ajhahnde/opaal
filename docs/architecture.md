...
Each activated module executes through the existing session driver with an isolated lexical root. Completed exports are cloned into importer roots as immutable snapshots; private bindings never become ambient names. Working directory, child-process environment, status, output, process activity, and background jobs remain shared across the whole program. Callable values retain their defining source so imported bodies and cross-file call frames are evaluated and rendered against the correct registered sources. Checker, formatter, help, editor, and language-server exposure remain later frontends over the shared program; type/signature and assignment-mutability analysis also remain separate.
