...
use opaal_platform::{Platform, ProcessStatus, SpawnRequest};
use opaal_platform_posix::PosixPlatform;

const SIGINT: i32 = 2;

fn main() {
    let report = PathBuf::from(env::var_os("OPAAL_GUARD_REPORT").expect("report path is required"));
    let observer =
        PathBuf::from(env::var_os("OPAAL_GUARD_OBSERVER").expect("observer path is required"));
    let workspace =
        PathBuf::from(env::var_os("OPAAL_GUARD_WORKSPACE").expect("workspace is required"));
...
fn spawn_raising_child(observer: &Path, workspace: &Path) -> ProcessStatus {
    ...
    let environment = [
        ...
        (OsString::from("OPAAL_PROBE_RAISE"), OsString::from("2")),
    ];
