...
use opaal_lsp::workspace::{ChangeOutcome, DiagnosticPublishOutcome, DocumentError, Workspace};
use opaal_runtime::module::{ModuleProgramLoader, ModuleResolver, ModuleSourceLoader};
use opaal_syntax::{PositionEncoding, Severity};
...
#[test]
fn opaal_source_budget_publishes_one_visible_incomplete_analysis_diagnostic() {
...
fn canonical_aliases_share_one_owner_and_conflicting_opens_are_rejected() {
    ...
    let directory = TestDirectory::new();
    let target = directory.path("target.opaal");
    let alias = directory.path("alias.opaal");
...
fn changes_require_a_strictly_increasing_present_version() {
    let directory = TestDirectory::new();
    let uri = directory.uri("new.opaal");
    let mut workspace = Workspace::new();
    workspace
    ...
    let unknown = directory.uri("unknown.opaal");
    assert_eq!(
        workspace
...
fn diagnostic_snapshots_are_immutable_and_sort_roots_by_canonical_path() {
    let directory = TestDirectory::new();
    let later_uri = directory.uri("zeta.opaal");
    let earlier_uri = directory.uri("alpha.opaal");
    let mut workspace = Workspace::new();
    workspace
    ...
    let module = ModuleResolver::new(&snapshot)
        .resolve_root(&directory.path("zeta.opaal"))
        .unwrap();
    assert_eq!(
...
fn diagnostics_are_normalized_deduplicated_versioned_and_published_atomically() {
    let directory = TestDirectory::new();
    let consumer_uri = directory.uri("consumer.opaal");
    let library_uri = directory.uri("library.opaal");
    let main_uri = directory.uri("main.opaal");
    let mut workspace = Workspace::new();
    workspace
        .open(
            main_uri.clone(),
...
fn provisional_identity_recanonicalizes_when_a_new_path_becomes_an_alias() {
    ...
    let directory = TestDirectory::new();
    let alias = directory.path("future.opaal");
    let target = directory.path("created.opaal");
    ...
    assert_eq!(
        provisional,
        fs::canonicalize(&directory.0).unwrap().join("future.opaal")
    );
...
fn conflicting_recanonicalization_is_rejected_without_changing_owners() {
    ...
    let directory = TestDirectory::new();
    let first_path = directory.path("first.opaal");
    let second_path = directory.path("second.opaal");
    let target = directory.path("target.opaal");
...
fn closing_an_overlay_restores_read_only_disk_loading() {
    let directory = TestDirectory::new();
    let path = directory.path("main.opaal");
...
fn lexical_aliases_of_a_provisional_path_resolve_to_the_overlay() {
    ...
    let path = directory.path("new.opaal");
    let uri = DocumentUri::from_absolute_path(&path).unwrap();
    let mut workspace = Workspace::new();
    workspace
        .open(uri.clone(), 1, "let editor = 1\n".into())
    ...
    let alias = directory.path("nested").join("..").join("new.opaal");
