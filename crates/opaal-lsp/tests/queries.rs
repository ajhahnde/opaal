...
use opaal_lsp::protocol::request_failure_response;
use opaal_lsp::query::{RequestControl, RequestError, prepare_request};
...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("opaal-lsp-queries-{}-{serial}", std::process::id()));
...
fn semantic_completion_hover_and_signature_help_use_shared_program_data() {
    ...
    .unwrap();
    assert_eq!(completion[0]["label"], "library::greet");

    let call = root.find("library::greet('world')").unwrap() + "library::gr".len();
    let hover = request(
        &workspace,
        PositionEncoding::Utf16,
        &control,
    ...
    let glob_call = root.find("glob('*.opaal')").unwrap() + 1;
    ...
    let glob_argument = root.find("'*.opaal'").unwrap() + 2;
    ...
    let import_name = root.find("as library").unwrap() + "as ".len() + 2;
    let effects = request(
        &workspace,
        PositionEncoding::Utf16,
        &control,
...
fn definition_and_references_project_canonical_cross_file_locations() {
    ...
    let call = root.rfind("library::greet").unwrap() + "library::gr".len();
    let control = RequestControl::new();
