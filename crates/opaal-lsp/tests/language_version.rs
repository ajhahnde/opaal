...
use opaal_syntax::{LanguageIdentity, PositionEncoding, SourceFile, SourceId};
...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "opaal-lsp-language-version-{}-{serial}",
            std::process::id()
        ));
...
#[test]
fn open_opaal_imports_keep_their_own_language_diagnostics() {
    let directory = TestDirectory::new();
    fs::write(directory.path("main.opaal"), "language 1\n").unwrap();
    ...
    assert_eq!(workspace.language(), LanguageIdentity::OpaalV1);
    assert_eq!(
    ...
    assert_eq!(library.diagnostics()[0].code(), Some("OP2001"));
}

#[test]
fn opaal_document_formatting_uses_the_documents_explicit_major() {
    let directory = TestDirectory::new();
    fs::write(directory.path("main.opaal"), "language 1\n").unwrap();
...
#[test]
fn the_default_workspace_constructor_selects_opaal_language_one() {
    let workspace = Workspace::new();
    assert_eq!(workspace.language(), LanguageIdentity::OpaalV1);
}

#[test]
fn the_opaal_protocol_workspace_selects_language_one_before_opening_documents() {
    let directory = TestDirectory::new();
    let uri = directory.uri("main.opaal");
    let mut workspace = Workspace::new();
    ...
    assert_eq!(diagnostics.diagnostics()[0].code(), Some("OP2001"));
}
...
#[test]
fn opaal_workflow_operations_share_completion_hover_signature_and_generation() {
    let directory = TestDirectory::new();
