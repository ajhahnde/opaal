...
use opaal_syntax::{
    ...
    TextRange, completion_target, format_source_opaal,
};
...
fn completion(
        ...
        && let Some(module) = module_for_document(program, document)
    {
        ...
        for name in queries.qualified_name_candidates(module, target.prefix()) {
...
fn hover(
    ...
    let queries = program.semantic_queries(&commands);
    let markdown = if let Some(effects) =
        queries.import_effects_at(module, cursor).filter(|effects| {
            !effects.direct().occurrences().is_empty()
                || !effects.transitive().occurrences().is_empty()
        }) {
...
fn module_for_document<'a>(
    ...
    document: &OpenDocument,
) -> Option<&'a opaal_runtime::module::ModuleId> {
    program
        .sources()
        .entries()
...
fn render_effects(
    direct: &opaal_runtime::module::ModuleEffectSummary,
    transitive: &opaal_runtime::module::ModuleEffectSummary,
) -> String {
    format!(
...
fn effect_list(summary: &opaal_runtime::module::ModuleEffectSummary) -> String {
