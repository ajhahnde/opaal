...
use flash_cli::highlight::{HighlightKind, SyntaxHighlighter};
...
fn assert_lossless(source: &str, segments: &[flash_cli::highlight::HighlightSegment]) {
    assert_eq!(
        segments
            .iter()
            .map(flash_cli::highlight::HighlightSegment::text)
            .collect::<String>(),
        source
    );
...
fn assert_segment(
    segments: &[flash_cli::highlight::HighlightSegment],
    text: &str,
    kind: HighlightKind,
) {
    assert!(
