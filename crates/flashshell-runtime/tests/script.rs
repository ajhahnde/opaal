...
use flashshell_runtime::eval::{Clock, FakeClock};
use flashshell_runtime::plan::SessionOptions;
use flashshell_runtime::resolve::ExecutableProbe;
use flashshell_runtime::script::execute_chain_subshell;
...
fn isolated_chain_mixed_pipeline_processes_inherit_the_callers_group() {
    ...
    execute_chain_subshell(
        "<chain>",
        "^flashshell-status-fixture exit 0 | decode bytes | encode bytes",
        &cwd,
        &mut environment,
        &standard_registry(),
        &Probe::new([fixture]),
