...
use opaal_migrate::{MigrationFormat, NativeSourceReader, analyze_roots};
use opaal_syntax::{SourceFile, SourceId, VersionedParseOutcome, parse_opaal};

const AUTOMATION_CORPUS: &str =
    include_str!("../../../tests/opaal-foundation/migration/automation-corpus.tsv");
...
#[test]
fn complete_sixty_script_automation_inventory_and_standalone_samples_are_classified() {
    ...
    assert_eq!(
        paths
            ...
            .len(),
        paths.len(),
        "the preserved FlashOS automation inventory must not contain duplicates"
    );
    assert!(paths.iter().all(|path| path.ends_with(".fsh")));

    let repository = repository_root();
    for relative in [
        "history/flash-v1/scheduling/run-campaign.fsh",
        "history/flash-v1/exercises/run.fsh",
        "tests/opaal-foundation/migration/flash-v1/preserved.fsh",
    ] {
        ...
        assert!(
            ...
            "Flash 1 parse failure in {relative}"
        );
        assert!(
            report.sources[0]
                .findings
                .iter()
                .any(|finding| finding.code == "MIG3001"),
        ...
        for (source_index, migrated) in report.sources.iter().enumerate() {
            assert!(
                !migrated.findings.iter().any(|finding| {
                    (finding.code.starts_with("MIG3") || finding.code == "MIG2004")
                        && finding.severity == opaal_migrate::FindingSeverity::Required
            ...
            assert!(
                ...
                "safe edits for {} from {relative} do not parse as OPAAL 1: {parsed:?}",
                migrated.source_uri
            );
        }
    }
...
fn focused_import_fixture_has_depth_first_sources_and_parseable_edits() {
    let root = repository_root().join("tests/opaal-foundation/migration/imports/root.fsh");
    ...
    for (index, path) in [root, support].into_iter().enumerate() {
        ...
        assert!(
            matches!(parsed, VersionedParseOutcome::Complete(_)),
            "applied migration must parse as OPAAL 1: {parsed:?}"
        );
    }
}

fn apply_edits(source: &str, migrated: &opaal_migrate::MigrationSource) -> String {
...
fn cli_json_preserves_percent_encoded_source_uri_and_exact_statuses() {
    let repository = repository_root();
    let binary = env!("CARGO_BIN_EXE_opaal-migrate-flash-v1");
    let required = Command::new(binary)
        .current_dir(&repository)
        .args([
            "--format",
            "json",
            "tests/opaal-foundation/migration/source uri/naïve.fsh",
        ])
        .output()
        .unwrap();
    ...
    assert!(stdout.contains(
        "\"source_uri\":\"tests/opaal-foundation/migration/source%20uri/na%C3%AFve.fsh\""
    ));
    ...
    let clean = Command::new(binary)
        ...
        .args([
            "--format",
            "json",
            "tests/opaal-foundation/language/grammar/complete/directive-only.opaal",
        ])
        .output()
        .unwrap();
...
fn golden_workflow_reports_the_exact_preserved_source_migration() {
    let repository = repository_root();
    let relative = "tests/opaal-foundation/migration/flash-v1/preserved.fsh";
    let path = repository.join(relative);
    let before = fs::read(&path).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_opaal-migrate-flash-v1"))
        .current_dir(&repository)
    ...
    assert_eq!(
        ...
        concat!(
            "{\"schema\":2,\"source_product\":\"flash\",\"source_language\":1,",
            "\"target_product\":\"opaal\",\"target_language\":1,\"sources\":[{",
            "\"source_uri\":\"tests/opaal-foundation/migration/flash-v1/preserved.fsh\",",
            "\"digest\":\"sha256:a604ad1257b89169ad1c4add15f9a99a897fd3c19bdc42bb071d52dda4420beb\",",
            "\"detected_product\":\"flash\",\"detected_language\":1,",
            "\"target_product\":\"opaal\",\"target_language\":1,\"findings\":[{\"code\":\"MIG3001\",",
            "\"severity\":\"required\",\"start\":0,\"end\":0,",
            "\"message\":\"add 'language 1' for OPAAL source\",",
            "\"edit\":{\"start\":0,\"end\":0,\"replacement\":\"language 1\\n\\n\"}}],",
            "\"unresolved\":false}]}\n",
        )
    );
}
