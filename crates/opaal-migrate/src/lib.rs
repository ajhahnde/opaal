...
//! Deterministic, read-only analysis of explicit Flash 1 source graphs.
...
use opaal_syntax::migration::{lex, parse_with_control};
use opaal_syntax::{
    ...
    TokenKind, VersionedParseOutcome, detect_source_language, parse_opaal_with_control,
};
...
const SOURCE_PRODUCT: &str = "flash";
const SOURCE_LANGUAGE: u16 = 1;
const TARGET_PRODUCT: &str = "opaal";
const TARGET_LANGUAGE: u16 = 1;
...
pub struct MigrationSource {
    ...
    pub detected_language: Option<u16>,
...
impl MigrationReport {
    ...
    fn write_json(&self, output: &mut impl fmt::Write) -> fmt::Result {
        write!(
            output,
            "{{\"schema\":{},\"source_product\":\"{SOURCE_PRODUCT}\",\"source_language\":{SOURCE_LANGUAGE},\"target_product\":\"{TARGET_PRODUCT}\",\"target_language\":{TARGET_LANGUAGE},\"sources\":[",
            self.schema
        )?;
        for (source_index, source) in self.sources.iter().enumerate() {
            ...
            output.write_str(",\"detected_product\":")?;
            match &source.detected_product {
                Some(product) => write_json_string(output, product)?,
                None => output.write_str("null")?,
            }
            output.write_str(",\"detected_language\":")?;
            match source.detected_language {
                Some(language) => write!(output, "{language}")?,
                None => output.write_str("null")?,
            }
            output.write_str(",\"target_product\":")?;
            write_json_string(output, &source.target_product)?;
            write!(
                output,
                ",\"target_language\":{},\"findings\":[",
        ...
        match format {
            ...
            MigrationFormat::Json => {
                let mut output = format!(
                    "{{\"schema\":{SCHEMA_VERSION},\"complete\":false,\"source_product\":\"{SOURCE_PRODUCT}\",\"source_language\":{SOURCE_LANGUAGE},\"target_product\":\"{TARGET_PRODUCT}\",\"target_language\":{TARGET_LANGUAGE},\"error\":{{"
                );
                match self {
                    Self::Source {
                        source_uri,
...
impl<'reader, Reader: SourceReader> Graph<'reader, Reader> {
    ...
    fn visit(
        ...
        let parsed = ParsedSource::parse(&source, is_opaal, &mut self.usage, &self.limits)?;
...
impl ParsedSource {
    fn parse(
        ...
        let parsed = if is_opaal {
            match parse_opaal_with_control(source, &is_exhausted) {
                ControlledVersionedParseOutcome::Parsed(outcome) => match outcome {
                    VersionedParseOutcome::Complete(script) => Self::OpaalV1(script.into_script()),
            ...
            match parse_with_control(source, &is_exhausted) {
                ControlledParseOutcome::Parsed(outcome) => match outcome {
                    ParseOutcome::Complete(script) => Self::FlashV1(script),
                    ParseOutcome::Incomplete(incomplete) => Self::Invalid(parse_finding(
                        incomplete.span(),
                        "incomplete Flash 1 source",
                    )),
    ...
    fn script(&self) -> Option<&Script> {
        match self {
            Self::FlashV1(script) | Self::OpaalV1(script) => Some(script),
            Self::Invalid(_) => None,
        }
    }
...
fn diagnostic_finding(diagnostics: &[opaal_syntax::Diagnostic]) -> MigrationFinding {
    ...
    MigrationFinding {
        ...
        message: format!(
            "Flash 1 source cannot be classified: {}",
            diagnostic.message()
        ),
        edit: None,
    }
...
fn analyze_source(
    ...
    let (detected_product, detected_language) = match node.parsed {
        ParsedSource::OpaalV1(_) => (Some(TARGET_PRODUCT.to_owned()), Some(TARGET_LANGUAGE)),
        ParsedSource::FlashV1(_) => (Some(SOURCE_PRODUCT.to_owned()), Some(SOURCE_LANGUAGE)),
        ParsedSource::Invalid(_) => (None, None),
    };
    ...
    match &node.parsed {
        ...
        ParsedSource::OpaalV1(_) => {}
        ParsedSource::FlashV1(script) => {
            let (insertion, replacement) = language_insertion(node.source.text());
            findings.push(MigrationFinding {
                code: "MIG3001".to_owned(),
                severity: FindingSeverity::Required,
                start: insertion,
                end: insertion,
                message: "add 'language 1' for OPAAL source".to_owned(),
    ...
    Ok(MigrationSource {
        ...
        detected_product,
        detected_language,
        target_product: TARGET_PRODUCT.to_owned(),
...
fn language_insertion(source: &str) -> (usize, String) {
    ...
    match source.find('\n') {
        Some(end) => (end + 1, "language 1\n\n".to_owned()),
        None => (source.len(), "\nlanguage 1\n".to_owned()),
    }
}

const NEW_RESERVED_WORDS: [&str; 5] = ["action", "enum", "language", "task", "type"];
const OPAAL_V1_RESERVED_WORDS: [&str; 26] = [
...
fn analyze_reserved_names(
    ...
    for reserved in &scan.reserved_uses {
        findings.push(MigrationFinding {
            code: "MIG3002".to_owned(),
            severity: FindingSeverity::Required,
            start: reserved.start,
            end: reserved.end,
            message: format!(
                "preserve Flash 1 data name `{}` across the OPAAL 1 reserved-word boundary",
                reserved.spelling
            ),
            edit: Some(MigrationEdit {
                start: reserved.start,
...
fn rename_finding(span: Span, name: &str, replacement: &str) -> MigrationFinding {
    MigrationFinding {
        ...
        message: format!("rename Flash 1 binding `{name}` because it is reserved by OPAAL 1"),
        edit: Some(MigrationEdit {
            start: span.start(),
            end: span.end(),
...
fn collision_free_name(name: &str, occupied: &BTreeSet<String>) -> String {
    let base = format!("{name}_flash_v1");
    ...
    for import in imports {
        ...
        let conflict = import.names.iter().find_map(|name| {
            if NEW_RESERVED_WORDS.contains(&name.as_str()) {
                Some(format!("imported name `{name}` is reserved by OPAAL 1"))
        ...
        findings.push(MigrationFinding {
            code: "MIG3005".to_owned(),
            severity: FindingSeverity::Required,
            start: import.statement_span.start(),
            end: import.statement_span.end(),
            message: if unsafe_names {
                format!(
                    "Flash 1 import needs a qualified OPAAL 1 alias, but {}",
                    conflict.expect("an unsafe import has a conflict")
                )
            } else {
                format!("rewrite Flash 1 import through OPAAL 1 module alias `{alias}`")
            },
...
fn import_alias(path: &Path, occupied: &BTreeSet<String>) -> String {
    ...
    if OPAAL_V1_RESERVED_WORDS.contains(&base.as_str()) {
...
fn analyze_length_operation(
    ...
    for command in uses {
        findings.push(MigrationFinding {
            code: "MIG3003".to_owned(),
            severity: FindingSeverity::Required,
            start: command.span.start(),
            end: command.span.end(),
            message: "qualify the Flash 1 `length` operation through OPAAL 1 `std::value`"
                .to_owned(),
            edit: Some(MigrationEdit {
                start: command.span.start(),
                end: command.span.end(),
    ...
    findings.push(MigrationFinding {
        ...
        message: "import the qualified OPAAL 1 value operation module".to_owned(),
        edit: Some(MigrationEdit {
            start: insertion,
            end: insertion,
...
fn analyze_effects(
    ...
    for span in &effect_spans {
        findings.push(MigrationFinding {
            code: "AUTH3001".to_owned(),
            severity: FindingSeverity::Required,
            start: span.start(),
            end: span.end(),
            message: "ambient effectful execution is unavailable in the OPAAL 1 pure foundation"
                .to_owned(),
            edit: None,
        })?;
    }
...
mod tests {
    ...
    #[test]
    fn import_alias_avoids_every_opaal_keyword() {
        let reader = FakeReader::default()
            .source(
                "/project/root.fsh",
    ...
    fn reserved_binding_and_references_share_one_collision_free_rename() {
        let reader = FakeReader::default().source(
            "/project/root.fsh",
            "let action = 1\nlet action_flash_v1 = 2\n$action\n",
        );
        let report = analyze_roots(&reader, &[PathBuf::from("/project/root.fsh")]).unwrap();
        let edits = report.sources[0]
            .findings
            .iter()
            .filter(|finding| finding.code == "MIG3002")
            .map(|finding| finding.edit.as_ref().unwrap().replacement.as_str())
            .collect::<Vec<_>>();
        assert_eq!(edits, ["action_flash_v1_2", "action_flash_v1_2"]);
    }
    ...
    fn json_schema_has_stable_field_order_and_digest() {
        ...
        assert_eq!(
            rendered,
            "{\"schema\":2,\"source_product\":\"flash\",\"source_language\":1,\"target_product\":\"opaal\",\"target_language\":1,\"sources\":[{\"source_uri\":\"/project/root.fsh\",\"digest\":\"sha256:ccdab916a2664126bae77709d63891fea8e32421e01f33fa9814de8abe9ba3e3\",\"detected_product\":\"flash\",\"detected_language\":1,\"target_product\":\"opaal\",\"target_language\":1,\"findings\":[{\"code\":\"MIG3001\",\"severity\":\"required\",\"start\":0,\"end\":0,\"message\":\"add 'language 1' for OPAAL source\",\"edit\":{\"start\":0,\"end\":0,\"replacement\":\"language 1\\n\\n\"}}],\"unresolved\":false}]}"
        );
        assert_eq!(
            report.render(MigrationFormat::Human).unwrap(),
            "/project/root.fsh:0:0: required MIG3001: add 'language 1' for OPAAL source"
        );
    ...
    fn imported_command_spread_is_unresolved_without_an_unsafe_edit() {
        ...
        let import = root
            ...
            .iter()
            .find(|finding| {
                finding.code == "MIG3005" && finding.message.contains("used as a command spread")
            })
    ...
    fn host_intrinsics_and_commands_follow_source_order_and_authority() {
        ...
        let authority = root
            .findings
            .iter()
            .filter(|finding| finding.code == "AUTH3001")
            .collect::<Vec<_>>();
        let length_edits = root
            .findings
            .iter()
            .filter(|finding| {
                finding.code == "MIG3003"
                    && finding
                        .edit
                        .as_ref()
    ...
    fn deep_and_wide_import_limits_stop_before_the_excess_read() {
        let deep_reader = RecordingReader::default()
            ...
            .source("/project/c.fsh", "language 1\n");
        analyze_roots_with_limits(
            &deep_reader,
            &[PathBuf::from("/project/root.fsh")],
            &MigrationLimits {
        ...
        let wide_reader = RecordingReader::default()
            ...
            .source("/project/a.fsh", "language 1\n")
            .source("/project/b.fsh", "language 1\n")
            .source("/project/c.fsh", "language 1\n");
        analyze_roots_with_limits(
            &wide_reader,
            &[PathBuf::from("/project/root.fsh")],
    ...
    fn every_migration_resource_limit_accepts_the_boundary_and_refuses_first_excess() {
        ...
        let two_sources = FakeReader::default()
            .source("/project/root.fsh", "import './support.fsh'\n")
            .source("/project/support.fsh", "language 1\n");
