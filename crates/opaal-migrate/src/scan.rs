...
pub(crate) fn scan(source: &opaal_syntax::SourceFile, script: &Script) -> SourceScan {
...
fn text(source: &opaal_syntax::SourceFile, span: Span) -> String {
    source
        .slice(span)
...
fn binding(source: &opaal_syntax::SourceFile, span: Span, scan: &mut SourceScan) {
...
fn scan_statement(source: &opaal_syntax::SourceFile, statement: &Statement, scan: &mut SourceScan) {
    match statement.kind() {
        ...
        StatementKind::Environment(environment) => {
            ...
            match environment {
                ...
                opaal_syntax::EnvironmentStatement::Unset { .. } => {}
            }
        }
        ...
        StatementKind::Control(opaal_syntax::ControlTransfer::Return(Some(expression))) => {
...
fn scan_block(source: &opaal_syntax::SourceFile, block: &Block, scan: &mut SourceScan) {
    scan_scoped_statements(source, &block.statements, block.span.end(), scan);
}

fn scan_pattern(source: &opaal_syntax::SourceFile, pattern: &Pattern, scan: &mut SourceScan) {
    match pattern {
    ...
    match expression.kind() {
        ...
        ExpressionKind::Record(entries) => {
            for entry in entries {
                if let opaal_syntax::RecordKey::Identifier(identifier) = &entry.key {
...
fn scan_chain(source: &opaal_syntax::SourceFile, chain: &ConditionalChain, scan: &mut SourceScan) {
...
fn scan_word(source: &opaal_syntax::SourceFile, word: &Word, scan: &mut SourceScan) {
    for part in word.parts() {
        scan_word_part(source, part, scan);
    }
}

fn scan_word_part(source: &opaal_syntax::SourceFile, part: &WordPart, scan: &mut SourceScan) {
    match part.kind() {
...
fn simple_word(source: &opaal_syntax::SourceFile, word: &Word) -> Option<String> {
...
fn add_quoted_reserved_use(source: &opaal_syntax::SourceFile, span: Span, scan: &mut SourceScan) {
...
fn pattern_names(source: &opaal_syntax::SourceFile, pattern: &Pattern, names: &mut Vec<String>) {
    match pattern {
