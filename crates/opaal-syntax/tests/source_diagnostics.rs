...
fn source_spans_and_locations_use_original_utf8_bytes() {
    let source = SourceFile::new(SourceId::new(7), "examples/demo.opaal", "first\r\né bad\n");

    assert_eq!(source.id(), SourceId::new(7));
    assert_eq!(source.name(), "examples/demo.opaal");
    ...
    let other = SourceFile::new(SourceId::new(8), "other.opaal", "bad");
    assert!(matches!(
...
fn byte_loading_rejects_non_utf8_source() {
    assert!(SourceFile::from_bytes(SourceId::new(1), "bad.opaal", vec![0xff]).is_err());
}

#[test]
fn protocol_positions_are_checked_in_utf8_and_utf16_code_units() {
    let source = SourceFile::new(SourceId::new(2), "unicode.opaal", "aé💡\r\nnext\n");
...
fn protocol_positions_reject_partial_scalars_surrogates_and_out_of_range_values() {
    let source = SourceFile::new(SourceId::new(3), "unicode.opaal", "aé💡\r\n");
...
fn diagnostics_render_ordered_labels_notes_and_empty_spans() {
    let source = SourceFile::new(SourceId::new(3), "examples/demo.opaal", "first\r\né bad\n");
    ...
    let diagnostic = Diagnostic::new(Severity::Error, "OP1001", "unexpected word")
    ...
    assert_eq!(
        ...
        concat!(
            "error[OP1001]: unexpected word\n",
            " --> examples/demo.opaal:2:3\n",
            "  |\n",
            "2 | é bad\n",
            "  |   ^^^ not valid here\n",
            "1 | first\n",
...
fn diagnostic_rendering_rejects_a_label_from_another_source() {
    let source = SourceFile::new(SourceId::new(1), "one.opaal", "one");
    let other = SourceFile::new(SourceId::new(2), "two.opaal", "two");
    let diagnostic = Diagnostic::new(Severity::Warning, "OP2001", "cross-source label")
...
fn diagnostics_group_labels_from_multiple_sources() {
    let root = SourceFile::new(SourceId::new(1), "root.opaal", "import './lib.opaal'\n");
    let library = SourceFile::new(SourceId::new(2), "lib.opaal", "import './root.opaal'\n");
