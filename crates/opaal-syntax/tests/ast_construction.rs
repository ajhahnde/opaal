...
fn nodes_and_word_parts_retain_exact_source_spans() {
    ...
    let declaration = Statement::new(
        StatementKind::Declaration(opaal_syntax::Declaration {
            mutable: false,
            pattern: opaal_syntax::Pattern::Binding(Identifier::new(span(&source, 4..9))),
    ...
    let command = Statement::new(
        StatementKind::Job(opaal_syntax::JobStatement {
            chain: opaal_syntax::ConditionalChain::from_pipeline(
                opaal_syntax::Pipeline::from_stage(AstNode::new(
                    opaal_syntax::StageKind::Command(CommandStage {
    ...
    let opaal_syntax::StageKind::Command(stage) =
...
fn span(source: &SourceFile, range: std::ops::Range<usize>) -> opaal_syntax::Span {
