...
use flash_runtime::convert::{Codec, DecodeStep, EncodeStep, decode, encode};
...
fn decode_value(decoder: &mut flash_runtime::convert::Decoder) -> Value {
    match decoder.pull() {
...
fn encode_chunk(encoder: &mut flash_runtime::convert::Encoder) -> Vec<u8> {
    match encoder.pull() {
