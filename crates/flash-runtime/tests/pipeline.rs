...
use flash_runtime::eval::{Clock, Instant, RuntimeErrorKind};
use flash_runtime::execute::{execute_foreground_pipeline, start_background_pipeline};
use flash_runtime::plan::{ExecutionPlan, RedirectionAction, plan_pipeline};
        ...
        Ok(Box::new(RecordingChild {
            ...
            process_group: self
                ...
                .and_then(|placement| match placement {
                    ProcessGroup::New => flash_platform::ProcessGroupId::new(index as u64 + 1),
...
struct RecordingChild {
    index: usize,
    id: u64,
    process_group: Option<flash_platform::ProcessGroupId>,
...
impl ChildProcess for RecordingChild {
    ...
    fn process_group(&self) -> Option<flash_platform::ProcessGroupId> {
        self.process_group
    }
...
fn real_posix_pipeline_moves_more_than_a_pipe_buffer_without_capture() {
    ...
    let fixture = PathBuf::from(env!("CARGO_BIN_EXE_flash-stream-fixture"));
...
fn a_foreground_pipeline_owns_the_terminal_and_gives_it_back() {
    ...
    let interactive = FakePlatform::with_terminal(
        Capabilities::full(),
        true,
        flash_platform::TerminalSize::new(80, 24),
    );
