...
use flash_runtime::eval::{ExpandedWord, RuntimeErrorKind, expand_word};
use flash_runtime::{BindingMutability, ByteSize, Duration, ScopeError, ScopeStack, Value};
use flash_syntax::{
...
fn scalar_interpolation_encodes_each_family_canonically() {
    ...
    scope
        .declare(
            ...
            Value::Float(flash_runtime::FiniteFloat::new(1.5).unwrap()),
        )
        .unwrap();
    scope
...
fn path_interpolation_preserves_native_units() {
    ...
    scope
        .declare(
            "p",
            BindingMutability::Immutable,
            Value::Path(flash_runtime::NativePath::new("/etc/hosts")),
        )
        .unwrap();
    assert_eq!(
