...
fn collect_propagates_a_producer_failure() {
    ...
    let stream = ValueStream::from_fn(move || {
        ...
        match pulls {
            ...
            2 => Some(Err(flash_runtime::eval::RuntimeError::new(
                flash_runtime::eval::RuntimeErrorKind::ExecutionUnsupported,
                {
                    let file =
                        flash_syntax::SourceFile::new(flash_syntax::SourceId::new(1), "t", "x");
...
fn drain_lines(mut splitter: flash_runtime::structured::LineSplitter) -> Vec<String> {
    let mut out = Vec::new();
    loop {
        match splitter.pull() {
...
fn lines_passes_an_upstream_failure_through() {
    let file = flash_syntax::SourceFile::new(flash_syntax::SourceId::new(1), "t", "x");
...
fn drain_select(mut selector: flash_runtime::structured::SelectStream) -> Vec<Value> {
    let mut out = Vec::new();
    loop {
        match selector.pull() {
...
fn drain_get(mut getter: flash_runtime::structured::GetStream) -> Vec<Value> {
    let mut out = Vec::new();
    loop {
        match getter.pull() {
