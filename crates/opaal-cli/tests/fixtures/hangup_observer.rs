...
fn main() {
    ...
    let directory =
        PathBuf::from(env::var_os("OPAAL_PROBE_GROUP_REPORT").expect("report path is required"));
    let release =
        PathBuf::from(env::var_os("OPAAL_PROBE_HOLD_UNTIL").expect("release path is required"));
