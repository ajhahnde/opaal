...
fn assert_lossless(source: &str, segments: &[opaal_cli::highlight::HighlightSegment]) {
    assert_eq!(
        segments
            .iter()
            .map(opaal_cli::highlight::HighlightSegment::text)
            .collect::<String>(),
        source
    );
...
fn assert_segment(
    segments: &[opaal_cli::highlight::HighlightSegment],
    text: &str,
    kind: HighlightKind,
) {
    assert!(
