...
fn platform_paths_use_state_roots_and_disabled_mode_bypasses_discovery() {
    ...
    assert_eq!(
        ...
        HistorySelection::Persistent {
            path: PathBuf::from("/state/opaal/history"),
        }
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        HistorySelection::Persistent {
            path: PathBuf::from("/home/user/.local/state/opaal/history"),
        }
    );
    ...
    assert_eq!(
        ...
        HistorySelection::Persistent {
            path: PathBuf::from("/Users/user/Library/Application Support/opaal/history"),
        }
    );
...
fn disabled_history_records_nothing_and_persistent_history_is_exactly_bounded() {
    ...
    let path = directory.path().join("opaal/history");
...
#[test]
fn persistent_history_uses_only_the_canonical_opaal_path() {
...
impl TempDirectory {
    fn new(label: &str) -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("opaal-history-{label}-{}-{id}", std::process::id()));
