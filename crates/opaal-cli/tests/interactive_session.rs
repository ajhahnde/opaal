...
use opaal_cli::completion::{CompletionCatalog, CompletionEngine};
use opaal_cli::editor::{EditorError, EditorEvent, EditorPrompt, LineEditor};
...
use opaal_cli::report::HostExit;
use opaal_platform::ProcessGroupId;
use opaal_runtime::background::JobNoticeKind;
use opaal_runtime::background::{LiveJob, LiveJobState};
use opaal_runtime::builtin::standard_registry;
use opaal_runtime::job::JobId;
...
impl StatefulEvaluator {
    fn assert_seeded(&self) {
        assert_eq!(self.scope.as_deref(), Some("OPAAL"));
...
impl InteractiveEvaluator for StatefulEvaluator {
    fn evaluate(
        ...
        match source {
            "seed" => {
                self.scope = Some("OPAAL".to_owned());
...
fn fatal_interactive_exit_hangs_up_jobs_before_reporting_status_one() {
    ...
    assert_eq!(
        ...
        "opaal: interactive editor event is not supported: host command\n"
    );
}
...
fn a_failed_program_output_flush_is_a_fatal_interactive_failure() {
    ...
    assert_eq!(
        String::from_utf8(diagnostics.bytes).expect("fatal report should be UTF-8"),
        concat!(
            "opaal: interactive command output failed: scripted flush failure\n",
            "opaal:   caused by: scripted flush failure\n"
        )
    );
}
...
fn live_job_refusal_names_the_explicit_force_command_for_every_job() {
    ...
    assert!(rendered.ends_with("opaal: exit again to hang up\n"));
}
...
fn a_refused_exit_writes_the_refusal_and_keeps_the_session_alive() {
    ...
    let mut evaluator = GatedEvaluator::refusing_once("opaal: 1 live background job\n");
    let mut diagnostics = Vec::new();
