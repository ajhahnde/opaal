...
fn recursive_snapshots_are_bounded_exact_and_do_not_follow_symlinks() {
    ...
    fs::write(cwd.join("scripts/a.opaal"), b"").unwrap();
    fs::write(cwd.join("scripts/nested/b.opaal"), b"").unwrap();
    fs::write(cwd.join("scripts/nested/deep/c.opaal"), b"").unwrap();
    fs::write(cwd.join("space name"), b"").unwrap();
    fs::write(cwd.join("quote'name"), b"").unwrap();
    fs::write(cwd.join("🚀.opaal"), b"").unwrap();
