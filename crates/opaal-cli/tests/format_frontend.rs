...
use opaal_syntax::{
    Diagnostic, FormatOutcome, Severity, SourceFile, SourceId, format_source_opaal,
    render_diagnostic,
};
...
fn check_accumulates_all_failures_and_noncanonical_sources_in_operand_order() {
    ...
    assert_eq!(
        failure_paths(&run),
        vec![
            PathBuf::from("noncanonical.opaal"),
            PathBuf::from("unicode-docs.opaal"),
            PathBuf::from("incomplete.opaal"),
            PathBuf::from("invalid.opaal"),
            PathBuf::from("non-utf8.opaal"),
            PathBuf::from("unreadable.opaal"),
            PathBuf::from("directory.opaal"),
            PathBuf::from("symlink.opaal"),
            PathBuf::from("late-noncanonical.opaal"),
            PathBuf::from("alias.opaal"),
        ]
    );
    ...
    assert_eq!(
        ...
        "opaal format: unreadable.opaal: read: permission denied\n"
    );
    ...
    assert!(run.failures()[9].rendered().contains("canonical.opaal"));
...
fn write_preflight_failure_prevents_every_replacement() {
    ...
    let run = format_files(
        &FormatRequest::new(
            ...
            ["first.opaal", "broken.opaal", "last.opaal"].map(PathBuf::from),
        ),
        &mut filesystem,
    );

    assert!(!run.is_success());
    assert_eq!(failure_paths(&run), vec![PathBuf::from("broken.opaal")]);
    assert!(filesystem.replacements().is_empty());
}
...
fn write_skips_unchanged_sources_and_replaces_changed_sources_in_order() {
    ...
    let run = format_files(
        &FormatRequest::new(
            FormatOperation::Write,
            ["unchanged.opaal", "first.opaal", "second.opaal"].map(PathBuf::from),
        ),
        &mut filesystem,
    );
...
fn write_stops_at_the_first_replacement_failure() {
    let mut filesystem = FakeFilesystem::default();
    for name in ["first.opaal", "failing.opaal", "untouched.opaal"] {
        filesystem.insert(
            name,
            Entry::regular(
    ...
    filesystem.fail_replacement("failing.opaal", "atomic rename failed");

    let run = format_files(
        &FormatRequest::new(
            FormatOperation::Write,
            ["first.opaal", "failing.opaal", "untouched.opaal"].map(PathBuf::from),
        ),
        &mut filesystem,
    );

    assert!(!run.is_success());
    assert_eq!(failure_paths(&run), vec![PathBuf::from("failing.opaal")]);
    assert_eq!(
        run.failures()[0].rendered(),
        "opaal format: failing.opaal: replace: atomic rename failed\n"
    );
    assert_eq!(
        filesystem
            .replacements()
            ...
            .collect::<Vec<_>>(),
        vec![PathBuf::from("first.opaal"), PathBuf::from("failing.opaal")]
    );
    assert_eq!(
...
fn failure_paths(run: &opaal_cli::format::FormatRun) -> Vec<PathBuf> {
    run.failures()
        .iter()
...
fn expected_fmt001(path: &str, original: &str) -> String {
    ...
    .with_note(format!(
        "run `opaal format --write -- {path}` to rewrite this source"
    ));
...
fn expected_incomplete(path: &str, text: &str) -> String {
    ...
    let FormatOutcome::Incomplete(incomplete) = format_source_opaal(&source) else {
...
fn expected_invalid(path: &str, text: &str) -> String {
    let source = SourceFile::new(SourceId::new(1), path, text);
    let FormatOutcome::Invalid(diagnostics) = format_source_opaal(&source) else {
