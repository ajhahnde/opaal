...
impl Pty {
    ...
    fn spawn_with_env(binary: &str, environment: &[(&str, &str)]) -> Self {
        ...
        // so terminal raw mode and key events reach it.
...
fn external_output_redraws_a_live_portable_edit_buffer() {
    let mut pty = Pty::spawn_with_env(FIXTURE, &[("OPAAL_TEST_EXTERNAL_NOTICE", "1")]);
    pty.wait_for(">> ");
    let mark = pty.mark();
...
fn persistent_history_survives_portable_editor_processes() {
    ...
    let environment = [
        ("OPAAL_TEST_PERSISTENT_HISTORY", "1"),
...
fn configured_and_safe_prompts_reach_the_portable_editor() {
    let mut configured = Pty::spawn_with_env(
        FIXTURE,
        &[
            ("OPAAL_TEST_PROMPT", "opaal> "),
            ("OPAAL_TEST_CONTINUATION_PROMPT", "more> "),
        ],
    );
    configured.wait_for("opaal> ");
    configured.send(b"if true {\r");
    configured.wait_for("more> ");
    configured.send(b"}\r");
    configured.await_prompt_text_after("submitted: if true {", "opaal> ");
    ...
    let mut safe = Pty::spawn_with_env(
        FIXTURE,
        &[
            ("OPAAL_TEST_SAFE_MODE", "1"),
            ("OPAAL_TEST_PROMPT", "must-not-render> "),
            ("OPAAL_TEST_CONTINUATION_PROMPT", "must-not-render> "),
        ],
    );
    safe.wait_for("[SAFE] >> ");
