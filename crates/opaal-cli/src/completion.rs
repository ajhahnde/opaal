...
use opaal_runtime::command::{CommandClassification, CommandRegistry};
use opaal_runtime::glob::glob_pattern_matches;
use opaal_runtime::intrinsic::{DynamicBinding, ExpressionIntrinsic};
...
impl CompletionCatalog {
    ...
    pub fn from_runtime(registry: &CommandRegistry, scope: &ScopeStack) -> Self {
        ...
        for (name, value) in scope.visible_bindings() {
            ...
            if matches!(value, opaal_runtime::Value::Callable(callable) if callable.family() == "function")
            {
