...
/// Why a completed OPAAL status cannot be exposed as an eight-bit host status.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// Map one completed OPAAL status to the exact host status byte.
pub fn map_status(status: &Status) -> Result<u8, StatusMappingError> {
    map_status_fields(StatusFields {
        code: status.code(),
        signal_number: status.signal().map(opaal_runtime::Signal::number),
    })
}
    ...
    match report {
        ...
        HostReport::Completed {
            ...
            match map_status(status) {
                ...
                Err(error) => {
                    let rendered = format!("opaal: cannot represent completed status: {error}\n");
        ...
        HostReport::Misuse { message } => {
            ...
            let rendered = format!("opaal: {message}\n");
            let _ = write_required(diagnostics, rendered.as_bytes());
            HostExit::Misuse
        }
    }
...
fn report_output_failure(diagnostics: &mut (impl Write + ?Sized), error: &io::Error) {
    let rendered = format!("opaal: cannot write standard output: {error}\n");
    let _ = write_required(diagnostics, rendered.as_bytes());
}
...
mod tests {
    ...
    use opaal_runtime::{Duration, Signal, Status};
    ...
    fn completed_status_can_carry_ordered_shell_diagnostics() {
        ...
        let exit = write_report(
            HostReport::completed_with_diagnostic(
                &exited(7),
                b"",
                b"opaal: first background failure\nopaal: second background failure\n",
            ),
            &mut stdout,
            &mut stderr,
        );
    ...
    fn success_can_carry_ordered_secondary_diagnostics_without_becoming_failure() {
        ...
        let exit = write_report(
            HostReport::success_with_diagnostic(
                b"program output\n",
                b"opaal: completed evidence: cleanup\n",
            ),
            &mut stdout,
            &mut stderr,
        );
        ...
        assert_eq!(stdout.bytes, b"program output\n");
        assert_eq!(stderr.bytes, b"opaal: completed evidence: cleanup\n");
    }
    ...
    fn empty_success_failure_and_misuse_have_distinct_host_results() {
        ...
        assert_eq!(stderr.bytes, b"opaal: unexpected option\n");
    }
    ...
    fn mapping_failure_is_reported_once_on_stderr() {
        ...
        assert_eq!(
            ...
            "opaal: cannot represent completed status: exit code 256 is outside 0..=255\n"
        );
    }
        ...
        for fault in [Fault::Write, Fault::Flush] {
            ...
            assert!(
                String::from_utf8(stderr.bytes)
                    .unwrap()
                    .starts_with("opaal: cannot write standard output: injected ")
            );
        }
    }
