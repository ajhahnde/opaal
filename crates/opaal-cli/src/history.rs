//! OPAAL-owned policy and storage boundary for interactive history.
...
pub fn select_history(
        ...
        .or_else(|| {
            environment
                ...
                .map(|home| match platform {
                    HistoryPlatform::Linux | HistoryPlatform::Xdg => home.join(".local/state"),
    ...
    Ok(HistorySelection::Persistent {
        path: state_root.join("opaal/history"),
    })
}
...
fn open_history_file(path: &Path, create_new: bool) -> io::Result<File> {
    ...
    let descriptor = opaal_platform_posix::open_private_file_at(
        directory.as_fd(),
        name,
        create_new,
        HISTORY_FILE_MODE,
