...
use opaal_runtime::background::{JobNoticeKind, LiveJob, LiveJobState};
...
pub fn format_live_jobs(jobs: &[LiveJob]) -> String {
    ...
    rendered.push_str("opaal: exit again to hang up\n");
    rendered
}
...
fn render_fatal_report(cleanup_reports: &[String], error: &InteractiveSessionError) -> String {
    ...
    let _ = writeln!(rendered, "opaal: {error}");
    let mut source = error.source();
    while let Some(cause) = source {
        let _ = writeln!(rendered, "opaal:   caused by: {cause}");
        source = cause.source();
    }
    rendered
}
