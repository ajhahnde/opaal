...
impl SemanticQueries<'_> {
    ...
    /// Completes exported values and functions through a qualified local module alias.
    #[must_use]
    pub fn qualified_name_candidates(
        &self,
        module: &ModuleId,
        qualified_prefix: &str,
    ) -> Vec<VisibleName> {
        ...
        self.program
            .names()
            .exports(owner)
            .filter(|export| export.name().starts_with(name_prefix))
            .map(|export| {
                ...
                let function = self
                    .program
                    .types()
                    .function(owner, export.declaration_span())
                    .is_some();
                let value_type = if function {
                    ValueType::Function
                } else {
                    self.program
                        .types()
                        .binding_type(owner, export.declaration_span())
                        .cloned()
                        .unwrap_or(ValueType::Any)
                };
                VisibleName {
                    name: format!("{qualifier}::{}", export.name()),
                    kind: if function {
                        NameKind::ImportedFunction
                    } else {
                        NameKind::ImportedBinding
                    },
...
struct CallFinder<'a> {
    offset: usize,
    best: Option<(&'a opaal_syntax::CallExpression, Span)>,
    best_command: Option<(&'a opaal_syntax::CommandStage, Span)>,
...
impl<'a> CallFinder<'a> {
    ...
    ) -> Option<(&'a opaal_syntax::CallExpression, Span)> {
    ...
    fn find_command(
        mut self,
        statements: &'a [Statement],
    ) -> Option<(&'a opaal_syntax::CommandStage, Span)> {
        self.statements(statements);
        self.best_command
    }
    ...
    fn if_statement(&mut self, statement: &'a opaal_syntax::IfStatement) {
        self.chain(&statement.condition);
        self.block(&statement.then_block);
        match &statement.else_branch {
            Some(ElseBranch::Block(block)) => self.block(block),
    ...
    fn literal(&mut self, literal: &'a opaal_syntax::Literal) {
