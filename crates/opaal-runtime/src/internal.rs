    ...
    for stage in plan.stages().iter().skip(start) {
        let PlannedResolution::Internal { canonical_name, .. } = stage.resolution() else {
            return Err(RuntimeError::new(
                // opaal-foundation-boundary(executor-invariant): Segmentation keeps external stages outside internal suffixes.
...
pub(crate) fn execute_stage(
    ...
    match name {
        ...
        _ => Err(RuntimeError::new(
            // opaal-foundation-boundary(executor-invariant): Registry planning admits only the complete standard command set.
            RuntimeErrorKind::Unsupported {
