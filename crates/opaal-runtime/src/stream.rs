...
//! cardinality, and runtime-only [`StreamOwnerId`]. Checked opaal consumers enforce
//! that schema, latch the first terminal state, and close the owned resource at
//! most once. Low-level consumers retain the original untyped [`StreamPull`]
//! boundary; they do not acquire an inferred opaal contract.
//!
