...
use opaal_platform::{Platform, WorkingDirectoryRequest};
...
pub fn execute_builtin(
    ...
    let PlannedResolution::Internal { canonical_name, .. } = stage.resolution() else {
        return Err(RuntimeError::new(
            // opaal-foundation-boundary(executor-invariant): External stages never enter the built-in executor after planning.
    ...
    let signature = registry.lookup(canonical_name).ok_or_else(|| {
        RuntimeError::new(
            // opaal-foundation-boundary(executor-invariant): Every planned internal stage retains its registry entry.
            RuntimeErrorKind::Unsupported {
    ...
    let command = standard_name(canonical_name).ok_or_else(|| {
        RuntimeError::new(
            // opaal-foundation-boundary(executor-invariant): Only standard commands enter this executor after resolution.
            RuntimeErrorKind::Unsupported {
