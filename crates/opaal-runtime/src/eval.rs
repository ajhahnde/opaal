...
//!
//! The evaluator owns OPAAL expression, statement, callable, and control-flow
...
/// Embedding code retains its value semantics; an execution owner passes
...
impl ResourceBudget {
    ...
    /// The complete default OPAAL evaluator contract.
    #[must_use]
...
impl EvalLimits {
    ...
    /// Limits for pure OPAAL evaluation under the configured resource owner.
    #[must_use]
    pub const fn pure_opaal(cancel: CancellationToken, budget: ResourceBudget) -> Self {
        Self {
            cancel,
            budget,
...
impl EvaluationHost for PureEvaluationHost<'_> {
    ...
    fn read_directory(
        ...
        // opaal-foundation-boundary(embedding-refusal): Pure evaluator entry points have no filesystem host.
...
impl Evaluator<'_, '_> {
    ...
    fn job(
        ...
        if job.background_span.is_some() {
            // opaal-foundation-boundary(embedding-refusal): The session job coordinator owns background launch.
    ...
    fn nominal_record(
        ...
        let Some(type_segment) = record.name.segments.last() else {
            // opaal-foundation-boundary(carrier-refusal): A malformed nominal constructor cannot produce a runtime value.
    ...
    fn unsupported(&self, feature: &'static str, span: Span) -> Abort {
        // opaal-foundation-boundary(carrier-refusal): Callers use this for explicit value and carrier refusals.
...
pub(crate) fn restore_callable(snapshot: CallableSnapshot) -> Result<Arc<dyn Callable>, String> {
    let parsed = match opaal_syntax::parse_opaal(&snapshot.source) {
        ...
        opaal_syntax::ParseOutcome::Invalid(_) => {
                            ...
                            command.items.iter().find_map(|item| match item.kind() {
                                ...
                                CommandItemKind::Redirection(redirection) => {
                                    match redirection.kind() {
                                        ...
                                        | RedirectionKind::File(opaal_syntax::FileRedirection {
                                            target,
                                            ..
...
mod tests {
    ...
    impl EvaluationHost for DirectoryHost {
        ...
        fn read_directory(
            ...
            Ok(Box::new(CountingDirectoryStream {
                entries: vec![
                    DirectoryEntry::new("a.opaal".into(), DirectoryEntryKind::File, Some(0)),
                    DirectoryEntry::new("b.opaal".into(), DirectoryEntryKind::File, Some(0)),
                ]
                .into_iter(),
    ...
    fn glob_entry_limit_fails_before_a_partial_batch_can_escape() {
        let source = Arc::new(SourceFile::new(SourceId::new(1), "glob.opaal", "glob('*')"));
