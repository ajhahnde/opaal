...
/// One immutable opaal module/type/operation help result backed by canonical data.
...
mod tests {
    ...
    fn catalog_uses_visible_binding_shadowing_and_omits_opaque_callables() {
        ...
        let other_source =
            SourceFile::new(SourceId::new(802), "main.opaal", "def other() { null }\n");
        ...
        assert_eq!(target[0].definition().unwrap().source(), "library.opaal");
