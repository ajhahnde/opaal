...
/// One deterministic resource dimension charged by OPAAL analysis.
...
/// Exact configured ceilings for one OPAAL root-closure analysis.
...
impl AnalysisLimits {
    ...
    /// No resource ceilings, retained only for low-level tests.
    #[must_use]
...
/// One canonical local or compiled-standard OPAAL module identity.
...
/// One required OPAAL alias bound to a singular canonical module identity.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Canonical alias and explicit re-export tables for a complete opaal program.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
...
/// A resolved value type from OPAAL's closed built-in annotation namespace.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ValueType {
    Any,
    Null,
...
impl<'a> TypeCollector<'a> {
    ...
    fn resolved_type_parameters(
        &self,
        parameters: &[opaal_syntax::TypeParameter],
    ...
    fn statement(&mut self, statement: &Statement) -> Result<(), Box<ModuleTypeError>> {
        ...
        match statement.kind() {
            ...
            StatementKind::Environment(environment) => match environment {
                opaal_syntax::EnvironmentStatement::Export { value, .. } => self.expression(value),
    ...
    fn literal(&mut self, literal: &opaal_syntax::Literal) -> Result<(), Box<ModuleTypeError>> {
        if self.control.is_cancelled() {
            return Ok(());
        }
...
impl<'a> SignatureValidator<'a> {
    ...
    fn function(
        &self,
        function: &opaal_syntax::FunctionDefinition,
    ...
    fn statement(&self, statement: &Statement) -> Result<(), Box<ModuleTypeError>> {
        ...
        match statement.kind() {
            ...
            StatementKind::Environment(environment) => match environment {
                opaal_syntax::EnvironmentStatement::Export { value, .. } => {
    ...
    fn assignment(
        &self,
        assignment: &opaal_syntax::Assignment,
    ) -> Result<(), Box<ModuleTypeError>> {
    ...
    fn nominal_record_type(
        &self,
        record: &opaal_syntax::NominalRecordExpression,
    ...
    fn validate_match(
        &self,
        statement: &opaal_syntax::MatchStatement,
    ...
    fn nominal_for_qualified(&self, name: &opaal_syntax::QualifiedName) -> Option<&NominalType> {
    ...
    fn unknown_standard_operation(&self, name: &opaal_syntax::QualifiedName) -> Option<String> {
        let (operation, modules) = name.segments.split_last()?;
        if modules.is_empty() {
            return None;
        }
    ...
    fn nominal_for_qualified_prefix(
        &self,
        constructor: &opaal_syntax::QualifiedName,
    ) -> Option<&NominalType> {
...
impl<'a> ReferenceResolver<'a> {
    ...
    fn literal(&mut self, literal: &opaal_syntax::Literal) -> Result<(), Box<ModuleNameError>> {
        if self.control.is_cancelled() {
            return Ok(());
        }
        if let LiteralKind::DoubleQuoted(parts) = literal.kind() {
    ...
    fn qualified(
        &mut self,
        qualified: &opaal_syntax::QualifiedName,
    ) -> Result<(), Box<ModuleNameError>> {
        let Some((terminal, module_segments)) = qualified.segments.split_last() else {
            return Ok(());
        };
        if module_segments.is_empty() {
            return Ok(());
        }
        let modules = module_segments
            .iter()
            .map(|segment| self.text(segment.span()))
            .collect::<Vec<_>>();
        let Some(owner) = self.aliases.resolve(self.entry.module(), &modules) else {
        ...
        let name = self.text(terminal.span()).to_owned();
        let Some(export) = self.registry.export(owner, &name) else {
            return Ok(());
        };
        let Some(alias) = self.aliases.alias(self.entry.module(), modules[0]) else {
            return Ok(());
        };
        self.references.push(ModuleNameReference {
            name: qualified
...
impl<'a> StaticEffectAnalyzer<'a> {
    ...
    fn if_statement(&mut self, statement: &opaal_syntax::IfStatement) {
        if self.control.is_cancelled() {
            return;
        }
        self.chain(&statement.condition);
    ...
    fn command(&mut self, command: &opaal_syntax::CommandStage, is_last: bool) {
        if self.control.is_cancelled() {
            return;
        }
        self.word(command.head.word());
...
/// A completely loaded, parsed, and canonically graphed OPAAL source program.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl ModulePipelineError {
    ...
    fn primary_span(&self) -> Span {
        self.diagnostic
            .labels()
            .iter()
            .find(|label| label.style() == opaal_syntax::LabelStyle::Primary)
...
/// Recursively resolves, reads, decodes, parses, and graphs OPAAL modules.
...
impl<'a> ModuleProgramLoader<'a> {
    ...
    fn discover_module(
        ...
        let script = match parse_opaal_source(&source, &|| control.is_cancelled()) {
...
impl SyntaxMetrics {
    ...
    fn statement(&mut self, statement: &Statement) {
        ...
        match statement.kind() {
            ...
            StatementKind::Environment(opaal_syntax::EnvironmentStatement::Export {
                value,
                ..
            }) => {
                self.expression(value);
            }
            StatementKind::Environment(opaal_syntax::EnvironmentStatement::Unset { .. }) => {}
            StatementKind::Function(function) => {
                for parameter in &function.parameters {
...
/// A failure in the closed opaal module-alias and re-export namespace.
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub enum ModuleProgramError {
    ...
    /// A loaded module was incomplete or invalid OPAAL syntax.
    Syntax {
        module: ModuleId,
    ...
    /// A opaal alias, standard module, or explicit alias re-export was invalid.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Syntax { module, .. } => write!(
                formatter,
                "module `{}` contains invalid OPAAL syntax",
                module.path().display()
            ),
