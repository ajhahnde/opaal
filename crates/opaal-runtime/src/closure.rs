...
//! `where` a [`WhereStream`], and [`update`] an [`UpdateStream`], each
...
use opaal_syntax::{SourceFile, Span};
...
/// A pull-driven `where` (filter) transformer produced by the `where` operation.
