...
use opaal_runtime::eval::{CancelReason, CancellationToken};
use opaal_runtime::stream::ValueStream;
use opaal_runtime::structured::{
...
fn collect_propagates_a_producer_failure() {
    ...
    let stream = ValueStream::from_fn(move || {
        ...
        match pulls {
            ...
            2 => Some(Err(opaal_runtime::eval::RuntimeError::new(
                opaal_runtime::eval::RuntimeErrorKind::ExecutionUnsupported,
                {
                    let file =
                        opaal_syntax::SourceFile::new(opaal_syntax::SourceId::new(1), "t", "x");
...
fn drain_lines(mut splitter: opaal_runtime::structured::LineSplitter) -> Vec<String> {
    let mut out = Vec::new();
    loop {
        match splitter.pull() {
...
fn lines_passes_an_upstream_failure_through() {
    let file = opaal_syntax::SourceFile::new(opaal_syntax::SourceId::new(1), "t", "x");
    let boom = opaal_runtime::eval::RuntimeError::new(
        opaal_runtime::eval::RuntimeErrorKind::ExecutionUnsupported,
        file.span(0..1).unwrap(),
    );
...
fn drain_select(mut selector: opaal_runtime::structured::SelectStream) -> Vec<Value> {
    let mut out = Vec::new();
    loop {
        match selector.pull() {
...
fn drain_get(mut getter: opaal_runtime::structured::GetStream) -> Vec<Value> {
    let mut out = Vec::new();
    loop {
        match getter.pull() {
