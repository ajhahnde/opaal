...
fn outcome_manifest_checks_compiled_variants_and_conversion_boundaries() {
    ...
    for (index, row) in manifest.lines().enumerate() {
        ...
        let report = ModuleProgramLoader::new(&modules, &modules).analyze(&root.join(fields[1]));
        match fields[0] {
...
fn standard_result_and_option_share_canonical_nominal_identity() {
    let program = load("complete/reference.opaal");
...
fn completed_values_domain_errors_and_statuses_remain_distinct() {
    ...
    let domain_result = execute(
        &load("complete/domain-error.opaal"),
        &NoExecutables,
        Environment::new(),
    );
    ...
    let option = execute(
        &load("complete/option-none.opaal"),
        &NoExecutables,
        Environment::new(),
    );
    ...
    let status = execute(
        &load("complete/status-success.opaal"),
        &NoExecutables,
        Environment::new(),
    );
    assert_eq!(status.value(), &Value::Null);
    assert_eq!(
        status.status().and_then(opaal_runtime::Status::code),
        Some(7)
    );
}

#[test]
fn pure_opaal_process_execution_refuses_before_probe_or_spawn() {
    let program = load("refused/process.opaal");
...
fn outcome_precedence_retains_one_primary_and_ordered_secondary_evidence() {
    let source = SourceFile::new(SourceId::new(1_701), "matrix.opaal", "effect");
...
fn cancellation_refusal_and_fatal_host_failure_are_distinct_primary_tags() {
    let source = SourceFile::new(SourceId::new(1_702), "tags.opaal", "tag");
    let span = source.span(0..3).unwrap();
    let cancellation =
        opaal_runtime::eval::Cancellation::new(opaal_runtime::eval::CancelReason::Requested, span);
    let cancelled =
        ExecutionOutcome::<(), &str>::new(PrimaryOutcome::Cancelled(cancellation), Vec::new());
    assert!(matches!(
        cancelled.primary(),
        PrimaryOutcome::Cancelled(value)
            if value.reason() == opaal_runtime::eval::CancelReason::Requested
    ));
...
fn load(path: &str) -> ModuleProgram {
    let modules = FixtureModules;
    ModuleProgramLoader::new(&modules, &modules)
