...
use opaal_runtime::eval::{CancelReason, CancellationToken, RuntimeError, RuntimeErrorKind};
use opaal_runtime::format::{
    FromJsonStep, FromTextStep, JsonMode, ToJsonStep, ToTextStep, from_json, from_text, to_json,
    to_text,
};
use opaal_runtime::stream::ValueStream;
use opaal_runtime::{Record, Table, Value};
...
fn a_document_split_across_chunks_is_joined_before_parsing() {
    ...
    assert_eq!(
        parser.pull_value(),
        Value::from(record(vec![("name", Value::string("opaal"))]))
    );
}
...
fn json_output_renders_every_encodable_family_and_refuses_the_rest() {
    ...
    for value in [
        ...
        Value::from(opaal_runtime::Range::new(0, 3, false)),
    ] {
...
fn both_directions_pass_upstream_failure_and_cancellation_through() {
    let file = SourceFile::new(SourceId::new(1), "test.opaal", "x");
...
fn text_line(parser: &mut opaal_runtime::format::FromText) -> String {
    match parser.pull() {
...
fn text_output_accepts_exactly_the_families_that_are_word_eligible() {
    ...
    for value in [
        ...
        Value::Int(1),
        Value::string("text"),
        Value::from(opaal_runtime::ByteSize::new(4)),
    ] {
        let family = value.family_name();
