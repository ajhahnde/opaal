...
use opaal_runtime::eval::{FakeClock, RuntimeError, RuntimeErrorKind};
...
use opaal_runtime::operation::{OperationInputType, OperationPurity, OperationStreamPrimary};
...
fn operation_identity_overloads_help_and_budgeted_stream_share_one_descriptor() {
    ...
    let program = ModuleProgramLoader::new(&modules, &modules)
        .load(&root.join("complete/reference.opaal"))
    ...
    let outcome = direct.execute_value_stream(
        ...
        .with_cancellation(opaal_runtime::eval::CancellationToken::from_fn({
            let cancelled = Arc::clone(&cancelled);
    ...
    assert!(matches!(
        outcome.primary(),
        OperationStreamPrimary::Cancelled(opaal_runtime::eval::CancelReason::Requested)
    ));
    ...
    let producer_source = SourceFile::new(SourceId::new(900), "producer.opaal", "x");
...
fn operation_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("tests/opaal-foundation/language/operations")
}
