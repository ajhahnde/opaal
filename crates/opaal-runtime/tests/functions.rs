...
fn closures_are_values_that_capture_and_call() {
    // A closure stored in a binding is called through its bare name.
...
fn callable_display_reveals_kind_and_origin() {
    ...
    assert_eq!(format!("{}", ok("{|x| x}")), "<closure at test.opaal:1:1>");
    assert_eq!(
        format!("{:?}", ok("{|x| x}")),
        "<closure at test.opaal:1:1>"
    );
}
