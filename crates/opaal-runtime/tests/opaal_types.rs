...
#[test]
fn opaal_type_manifest_executes_through_shared_analysis() {
    let root = type_root();
...
#[test]
fn dynamic_opaal_type_boundaries_fail_at_runtime_with_exact_errors() {
    let root = type_root();
    let manifest = fs::read_to_string(root.join("manifest.tsv")).unwrap();
    let modules = FixtureModules;
...
fn cross_module_nominal_annotations_keep_one_semantic_identity() {
    ...
    let modules = FixtureModules;
    let program = ModuleProgramLoader::new(&modules, &modules)
        .load(&root.join("complete/cross-module-main.opaal"))
    ...
    assert!(
        definition
            .module()
            .path()
            .ends_with("complete/support/model.opaal")
    );
}
...
fn named_function_signatures_expose_only_empty_downstream_slots() {
    ...
    let modules = FixtureModules;
    let program = ModuleProgramLoader::new(&modules, &modules)
        .load(&root.join("complete/generic-inference.opaal"))
...
fn constructors_and_patterns_share_nominal_semantic_queries_through_reexports() {
    ...
    let modules = FixtureModules;
    let program = ModuleProgramLoader::new(&modules, &modules)
        .load(&root.join("complete/reexport-main.opaal"))
...
#[test]
fn complete_opaal_type_corpus_executes_nominal_values_and_recursive_patterns() {
    let root = type_root();
    let manifest = fs::read_to_string(root.join("manifest.tsv")).unwrap();
    let modules = FixtureModules;
...
fn type_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("tests/opaal-foundation/language/types")
}
