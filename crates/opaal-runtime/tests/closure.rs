...
use opaal_runtime::closure::{EachStep, UpdateStep, WhereStep, each, update, r#where};
...
fn each_reports_a_non_callable_closure() {
    let file = SourceFile::new(SourceId::new(2), "c.opaal", "1");
    let limits = EvalLimits::default();
    let mut env = Environment::new();
    let mut mapper = each(
        ints(&[1]),
...
fn each_passes_an_upstream_failure_through() {
    ...
    let boom_file = SourceFile::new(SourceId::new(3), "b.opaal", "x");
...
fn plain_file() -> SourceFile {
    SourceFile::new(SourceId::new(9), "u.opaal", "0")
}
